"""Unsupervised EmbedKit refinement + KNN/LogReg evaluation on MHIST.

Models: ResNet18 (128-d), CONCH (512-d), UNI (1024-d)
Labels: HP=0 / SSA=1  — used ONLY for downstream evaluation, never for refinement.
...
For each model the script:
  ...
  3. Fits EmbedKit in self_supervised mode on train+val split (no labels).
  4. Re-evaluates on refined embeddings.
  5. Saves a comparison table to mhist_analysis/unsupervised_results.csv.

Usage:
    python examples/unsupervised_refine_mhist.py
"""
...
for model_name, npz_path in MODELS.items():
    ...
    # ── Self-supervised refinement (no labels) ───────────────────────────
    print(f"  Fitting EmbedKit self_supervised (epochs={EPOCHS}) ...", flush=True)
    t0 = time.time()
    ek = EmbedKit(
    ...
    # Labels intentionally omitted — purely unsupervised
    X_train_ref = ek.fit_transform(X_train)
    ...
    out_npz = OUT_DIR / f"unsup_refined_{model_name.lower()}.npz"
...
for clf in ("KNN", "LR"):
    for metric in ("acc", "f1", "roc_auc"):
        df[f"Δ_{clf}_{metric}"] = df[f"refined_{clf}_{metric}"] - df[f"raw_{clf}_{metric}"]
...
print("\n\n" + "="*70)
print("  MHIST UNSUPERVISED REFINEMENT — RESULTS COMPARISON".center(70))
print("="*70)
...
for clf_label in ("KNN", "LR"):
    ...
    print(df_print[cols].round(4).to_string())

csv_out = OUT_DIR / "unsupervised_results.csv"
